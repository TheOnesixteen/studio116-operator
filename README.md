...
The inbox shows task counts, current unresolved work, older unresolved work, review items that need approve/reject attention, failed task summaries, running task heartbeat context, queued tasks, and active lock summaries. Tasks untouched for 3+ days appear under Older unresolved work. Running task heartbeat is shown as extra context but is not used for staleness classification. The inbox does not mutate tasks, clean locks, approve or reject work, or print raw worker stdout/stderr.
